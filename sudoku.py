        ...
        for n in self.needed:
            ...
            for sq in self.unsolved():
                if n in sq.poss:
                    rows.append(sq.row)
                    cols.append(sq.col)
            # somehow it got here with an empty rows list... not sure how
            if rows and (len(rows) == rows.count(rows[0])): # all the elements are the same
                return ('row', rows[0], n)
            elif cols and (len(cols) == cols.count(cols[0])):
                return ('col', cols[0], n)
...
class Game():
    ...
    def in_line(self, n=1, verbose=False):
        '''checks every box to see if they have a given number in only a given
...
# extend in_line to include all groupings - 
# if one of those two in that box is a 4 by way of 
# this column, then nothing else in that box is a 4
# also possibly combine with one_or_other?